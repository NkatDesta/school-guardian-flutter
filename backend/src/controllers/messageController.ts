...
export class MessageController {
  async getMessages(req: any, res: Response): Promise<void> {
    try {
      ...
      // Enforce messaging constraints: Only Guardian ↔ Homeroom Teacher
      ...
      if (userRole === UserRole.GUARDIAN) {
        // Guardian can only see messages with homeroom teachers
        query = `
          ...
          AND ((m.sender_id = ? AND u.role = 'homeroom_teacher') OR (m.receiver_id = ? AND u.role = 'homeroom_teacher'))
        ...
        // Homeroom teacher can only see messages with guardians
        query = `
          SELECT m.message_id, m.sender_id, m.receiver_id, m.content, m.sent_at, m.is_read, m.message_type,
                 u.full_name as sender_name, u.role as sender_role
          FROM messages m
  ...
  async sendMessage(req: any, res: Response): Promise<void> {
    try {
      ...
      const isValidRelationship = 
        (senderRole === UserRole.GUARDIAN && receiver.role === 'homeroom_teacher') ||
        (senderRole === UserRole.HOMEROOM_TEACHER && receiver.role === 'guardian');
      ...
      if (!isValidRelationship) {
        res.status(403).json({
          ...
          error: {
            ...
            message: 'Messages can only be sent between guardians and homeroom teachers',
          },
          timestamp: new Date().toISOString(),
        });
        return;
  ...
  async getConversations(req: any, res: Response): Promise<void> {
    try {
      ...
      if (userRole === UserRole.GUARDIAN) {
        // Guardian sees conversations with homeroom teachers
        query = `
        ...
        // Homeroom teacher sees conversations with guardians
        query = `
          SELECT DISTINCT u.user_id, u.full_name, u.role,
                 (SELECT m.content FROM messages m 
                  WHERE ((m.sender_id = ? AND m.receiver_id = u.user_id) OR 
